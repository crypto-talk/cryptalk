...
import { api, ApiCoin, ApiPost, Asset, connectInjectedWallet, linkInjectedWallet, Member, refreshSession } from "../lib/api";
...
type AuthMode = "login" | "signup" | "wallet" | "link";
...
export default function Home() {
  ...
  const connectWallet = async () => {
    setError("");
    try {
      const profile = connected ? await linkInjectedWallet() : await connectInjectedWallet();
  ...
  const authenticate = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      const profile = authMode === "signup"
        ? await api.signup(String(form.get("email")), String(form.get("password")), String(form.get("nickname")))
        : await api.emailLogin(String(form.get("email")), String(form.get("password")));
  ...
  return (
    <main className="app-shell">
      ...
      {walletOpen && <div className="modal-backdrop" onMouseDown={() => setWalletOpen(false)}><section className="modal wallet-modal auth-modal" onMouseDown={(e) => e.stopPropagation()} role="dialog" aria-modal="true" aria-labelledby="auth-title"><button className="modal-close" onClick={() => setWalletOpen(false)}>×</button><span className="modal-symbol">{authMode === "link" || authMode === "wallet" ? "◇" : "◎"}</span><h2 id="auth-title">{authMode === "signup" ? "회원가입" : authMode === "login" ? "로그인" : authMode === "link" ? "지갑 자산 인증" : "지갑으로 로그인"}</h2><p>{authMode === "link" ? "서명으로 지갑 소유권을 확인하고 자산 인증을 시작합니다." : authMode === "wallet" ? "기존 지갑 회원은 서명으로 바로 로그인할 수 있어요." : "일반 계정으로 참여하고 지갑은 필요할 때 연결하세요."}</p>{error && <p role="alert" className="auth-error">{error}</p>}{authMode === "login" || authMode === "signup" ? <form className="auth-form" onSubmit={authenticate}>{authMode === "signup" && <input name="nickname" aria-label="닉네임" placeholder="닉네임" minLength={2} maxLength={40} required />}<input name="email" type="email" aria-label="이메일" placeholder="이메일" autoComplete="email" required /><input name="password" type="password" aria-label="비밀번호" placeholder="비밀번호 (8자 이상)" minLength={8} maxLength={72} autoComplete={authMode === "signup" ? "new-password" : "current-password"} required /><button type="submit">{authMode === "signup" ? "계정 만들기" : "로그인"}</button></form> : <div className="wallet-options"><button onClick={connectWallet}><span className="wallet-icon fox">M</span><strong>EVM 지갑</strong><small>MetaMask · Coinbase Wallet</small><b>→</b></button></div>}<div className="auth-switch">{authMode === "login" && <><button type="button" onClick={() => setAuthMode("signup")}>회원가입</button><span>·</span><button type="button" onClick={() => setAuthMode("wallet")}>지갑으로 로그인</button></>}{authMode === "signup" && <button type="button" onClick={() => setAuthMode("login")}>이미 계정이 있어요</button>}{authMode === "wallet" && <button type="button" onClick={() => setAuthMode("login")}>이메일로 로그인</button>}</div><small className="terms-copy">지갑 서명에는 거래나 수수료가 발생하지 않습니다.</small></section></div>}
